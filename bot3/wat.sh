...
find=$(curl http://www.watthakhanun.com/webboard/showthread.php?t=5230'&'page=$page_max | iconv -f tis-620 -t utf-8 | grep "รายการที่ ๗๐" | wc -l)
#find=$("$findx" | wc -l)
echo -e  "number :  $find"
if [[ find   -gt 0 ]] ; then
	printf "find"
	if [[ $(cat /home/pi/bot/bot3/wattemp) -ne page_max  ]] ;  then
		echo -e "\nNew"
		echo page max : $page_max find : $find  | mail -s "ลูกอม" lineemb@gmail.com
		$(omxplayer /home/pi/bot/bot3/h.mp3)
		$(echo $page_max > /home/pi/bot/bot3/wattemp )
	else 
		echo -e "\nDup"		
	fi
	else
